...
# __all__ = ['GCPBackBroker', 'GCPBrokerBack']


class GCPBackBroker(bt.BrokerBase):
    '''Broker Simulator
    ...
    def _try_exec_limit(self, order, popen, phigh, plow, plimit, pvolume):
        if order.isbuy():
            if plimit >= popen and (int(pvolume) % 100 == 0):
            ...
            elif plimit >= plow and (int(pvolume) % 100 == 0):
            ...
            if plimit <= popen and (int(pvolume) % 100 == 0):
                ...
                p = self._slip_down(plimit, popen, doslip=self.p.slip_open,
                                    lim=True)
                self._execute(order, ago=0, price=p)
            elif plimit <= phigh and (int(pvolume) % 100 == 0):
                # day high above req price ... match limit price
                self._execute(order, ago=0, price=plimit)

    def _try_exec_stop(self, order, popen, phigh, plow, pcreated, pclose, pvolume):
        if order.isbuy():
            if popen >= pcreated and (int(pvolume) % 100 == 0):
                # price penetrated with an open gap - use open
                p = self._slip_up(phigh, popen, doslip=self.p.slip_open)
                self._execute(order, ago=0, price=p)
            elif phigh >= pcreated and (int(pvolume) % 100 == 0):
            ...
            if popen <= pcreated and (int(pvolume) % 100 == 0):
                ...
                self._execute(order, ago=0, price=p)
            elif plow <= pcreated and (int(pvolume) % 100 == 0):
                # price penetrated during the session - use trigger price
    ...
    def _try_exec(self, order):
        ...
        if pclose is None:
            pclose = data.close[0]
        pvolume = getattr(data, 'tick_volume', None)
        if pvolume is None:
            pvolume = data.volume[0]
        ...
        if order.exectype == Order.Market:
            ...
            self._try_exec_limit(order, popen, phigh, plow, pcreated, pvolume)
            ...
            self._try_exec_stop(order, popen, phigh, plow, pcreated, pclose, pvolume)
